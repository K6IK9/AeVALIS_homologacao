...
class PerguntaAvaliacao(models.Model):
    ...
    """
    TIPO_CHOICES = [
        ('likert', 'Escala de Concordância (Likert) - Discordo totalmente / Concordo totalmente'),
...
class AvaliacaoDocente(models.Model):
    ...
    turma = models.ForeignKey(
        Turma,
        on_delete=models.CASCADE,
        related_name="avaliacoes_docente"
    )
    professor = models.ForeignKey(
        PerfilProfessor,
        on_delete=models.CASCADE,
...
class RespostaAvaliacao(models.Model):
    ...
    def __str__(self):
        identificacao = f"Anônimo ({self.session_key[:8]})" if self.anonima else str(self.aluno)
    ...
    def valor_display(self):
        ...
        if self.valor_numerico is not None:
            if self.pergunta.tipo == 'likert':
                ...
                return f"{self.valor_numerico} - {escalas.get(self.valor_numerico, 'N/A')}"
...
class ComentarioAvaliacao(models.Model):
    ...
    def __str__(self):
        identificacao = f"Anônimo ({self.session_key[:8]})" if self.anonimo else str(self.aluno)
